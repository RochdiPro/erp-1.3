...
//** Error Msg */
  ...
  addReglement(){
    ...
    this.show++;
    if (this.show == 1 && this.id_modeP_typeTwo !== undefined){
      this.ligneOne = true;
      if(parseInt(this.price) <= parseInt(this.totalTTc)){   
        rest = Number(this.totalTTc - this.price).toFixed(3);
        this.addReglementFormGroup.controls['valueTwo'].setValue(rest);
        ...
        let pdf_devis = {
          ...
          content: [
            ...
            },
            {
              columns: [
                {
                  text: '' + this.custemerName.contact + '\n\n',
                  fontSize: 11,
                  color: 'black',
                  bold: false,
            ...
            },
            {
              columns: [
                {
                  text: '' + this.totalHTBrut + '\n\n',
                  fontSize: 11,
                  color: 'black',
                  bold: false,
                  relativePosition: {x:40, y:511}
                  },
                  {
  ...
  fetch(url).then(res => res.text).then( ()=>{
    ...
    formData.append('Detail',myDetail); 
       //** send data to the API */
        this.devisService.updateQuote(formData).subscribe((res)=>{
          Swal.fire( 
            {title :"Devis modifié avec succés.",
        }).then((result) => {
