...
import {  DialogOverview4gDialog, DialogOverviewArticleDialog, DialogOverviewChargeDialog, DialogOverviewSerieDialog, DialogOverviewSimpleDialog, DialogOverview_nlot, DialogSerieModifBonEntreeLocal } from '../../dialogue-achat/dialogue-achat.component';
import { BonEntreeService } from '../Service/bon-entree.service';
import { AjouterArticlesComponent } from 'src/app/wms/Stockage/dialog/ajouter-articles/ajouter-articles.component';
...
export class ModifierBonEntreeComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, public bonEntreeService: BonEntreeService, private route: ActivatedRoute, private router: Router, private http: HttpClient, private fb: FormBuilder, public dialog: MatDialog) {
    this.bonEntreeService.BonEntreeLocal(this.route.snapshot.params.id).subscribe((data) => {
      ...
      this.bonEntreeService.Fournisseur(this.bonEntreeLocalData.id_Fr).subscribe((data7) => {
        ...
        this.InformationsGeneralesForm.controls['Fournisseur'].setValue(this.nom_fr);
      })     
    })    
    ...
    dialogRef.afterClosed().subscribe(res => {
      ...
      if (res != undefined) {
        for (let i = 0; i < this.resultat_dialog.length; i++) {
          ...
          this.detail_produit =[]
          if (test == "0") { this.ajouter(this.resultat_dialog[i].id_Produit, this.resultat_dialog[i].prix, this.resultat_dialog[i].qte, this.resultat_dialog[i].remise, this.detail_produit); }
        }
      }
    });
  }
  ...
  ajouterFicheBonEntreeLocal() {
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        this.bonEntreeService.modifierBonEntreeLocal(formData).subscribe((data) => {
