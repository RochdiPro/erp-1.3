...
export class AjouterBonCommandeComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private fb: FormBuilder, private Service: BonCommandeService, public dialog: MatDialog, private datePipe: DatePipe) {
    this.Fournisseurs();
  ...
  ajouter(obj: any) {
    ...
    this.newAttribute.Totale_TTC = Number(this.newAttribute.Montant_TTC );
    this.newAttribute.Total_HT = Number(this.newAttribute.Prix_avec_remise * this.newAttribute.Quantite).toFixed(3);
    this.newAttribute.remise_u = Number(Number(this.newAttribute.PrixU )-Number( this.newAttribute.Prix_avec_remise)  ).toFixed(3);
  ...
  modifer_prix_remise(i: any, id: any, nom: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.fieldArray[i].PrixU = Number( this.a[0]).toFixed(3);
        this.fieldArray[i].Prix_avec_remise = (Number( this.a[0]) * (1 - (Number(this.a[1] )) / 100)).toFixed(3);
     
        this.fieldArray[i].Montant_HT = Number(Number(this.fieldArray[i].Prix_avec_remise) * Number(this.fieldArray[i].Quantite))
        this.Totale_TTC = (Number(this.fieldArray[i].Prix_avec_remise) / (1 + (Number(this.fieldArray[i].Tva)) / 100)).toFixed(3);
        this.Montant_Fodec = (this.fieldArray[i].Montant_HT * this.fieldArray[i].Fodec) / 100;
        this.fieldArray[i].Montant_Fodec = Number(this.Montant_Fodec);
    
        this.Montant_TVA = ((Number(this.fieldArray[i].Montant_HT) + Number(this.fieldArray[i].Montant_Fodec)) * this.fieldArray[i].Tva) / 100;
        this.fieldArray[i].Montant_TVA = Number(this.Montant_TVA);
        this.fieldArray[i].Prix_U_TTC = (((Number(this.fieldArray[i].Montant_HT) + Number(this.fieldArray[i].Montant_Fodec) + Number(this.fieldArray[i].Montant_TVA))) / Number(this.fieldArray[i].Quantite)).toFixed(3);
        this.fieldArray[i].Montant_TTC = Number(this.fieldArray[i].Prix_U_TTC) * Number(this.fieldArray[i].Quantite);
        this.fieldArray[i].Total_TVA = ((Number(this.fieldArray[i].Montant_TVA)) * (Number(this.fieldArray[i].Quantite))).toFixed(3);
        this.fieldArray[i].Totale_TTC = Number(this.fieldArray[i].Montant_TTC );
        this.fieldArray[i].Total_HT = Number(this.fieldArray[i].Prix_avec_remise * this.fieldArray[i].Quantite).toFixed(3);
        this.fieldArray[i].remise_u = Number(Number(this.fieldArray[i].PrixU )-Number( this.fieldArray[i].Prix_avec_remise)  ).toFixed(3);
...
// generate pdf  etranger
  ...
  generatePDF(id: any, date_Creation: any) {
    ...
    let def = {
      ...
      header: [
        {
          text: this.totalTTc+" "+this.InformationsGeneralesForm.get('Devise').value ,
          fontSize: 15,
          color: 'white',
          bold: true,
          relativePosition: { x: 480, y: 665 }
        },
        {
          text: ''+this.code_fr,
          fontSize: 10,
  ...
  generatePDF_local(id: any, date_Creation: any) {
    ...
    for (let i = 0; i < this.fieldArray.length; i++) {
      ...
      if (this.fieldArray[i].Montant_Fodec > 0) { 
        ch3 = ch3 + "Fodec 1 %  \n"; ch2 = ch2 + " + " + Number(this.fieldArray[i].Montant_Fodec).toFixed(3 ) + "\n" ; 
      }
      let res = Number (this.fieldArray[i].PrixU) -Number( this.fieldArray[i].Prix_avec_remise )
      if (this.fieldArray[i].remise > 0) { 
        ch3 = ch3 + "Remise " + this.fieldArray[i].remise + " %"; ch2 = ch2 + " -  " +  Number(res).toFixed(3) + "\n" 
      }
      ch2=ch2+"\n"+ Number(this.fieldArray[i].Totale_TTC).toFixed(3 )+"\n ________"
      obj.push( ch3  );
      obj.push({ text:ch2, alignment: 'right' });
    ...
    let def = {
      ...
      header: [
        ...
        },
        {
          ...
          fontSize: 10,
          color: 'black',
          relativePosition: { x: 320, y: 662 }
        },
        ,
        {
          text: ""+this.totalRemise ,
          fontSize: 10,
          color: 'black',
          relativePosition: { x: 180, y: 662 }
        },
        ,
        {
          text: ""+this.totalHT ,
        ...
        ,
        {
          ...
          fontSize: 10,
          color: 'black',
          relativePosition: { x: 70, y: 683 }
        }
        ,
        {
          text: ""+this.totalFodec ,
          fontSize: 10,
          color: 'black',
          relativePosition: { x: 180, y: 683 }
        },
        {
          text: '' + this.assiette19,
          fontSize: 10,
          color: 'black',
          relativePosition: { x: 105, y: 775 }
        },
        {
          text: '' + this.Montant19,
          fontSize: 10,
          color: 'black',
          relativePosition: { x: 105, y: 795 }
        },
        ,
        {
          ...
          fontSize: 10,
          color: 'black',
          relativePosition: { x: 200, y: 775 }
        },
        {
          text: '' + this.Montant13,
          fontSize: 10,
          color: 'black',
          relativePosition: { x: 200, y: 795 }
        },
        ,
        {
          text: '' + this.assiette7,
          fontSize: 10,
          color: 'black',
          relativePosition: { x: 290, y: 775 }
        },
        {
          text: '' + this.Montant7,
          fontSize: 10,
          color: 'black',
          relativePosition: { x: 290, y: 795 }
        },
  ...
  calcul() {
    ...
    for (var i = 0; i < this.fieldArray.length; i++) {
      ...
      total5 += Number(this.fieldArray[i].remise_u) * (Number(this.fieldArray[i].Quantite))
